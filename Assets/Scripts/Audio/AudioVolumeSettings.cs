...
public class AudioVolumeSettings : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (System.IO.File.Exists(Application.persistentDataPath + "/AudioSettings.json"))
        {
            LoadAudio();
        }
        else
    ...
    {
        StreamReader sr = new StreamReader(Application.persistentDataPath + "/AudioSettings.json");
    ...
    public void SaveAudio(float master, float music, float sfx)
    {
        ...
        StreamWriter sw = new StreamWriter(Application.persistentDataPath + "/AudioSettings.json");
        sw.Write(json);
        sw.Close();
