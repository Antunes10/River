...
~unlockImage(2)
Now well rested, they go back to the river.
Morning goes by fast as they keep sailing West.
...
=== ChoicePanel ===
++ {not TakeALook} [Take a look at the "metal fangs" and the bird]
    ->TakeALook
    
++ {not TryToOpen} [Try to open a gap for the bird to escape]
    ->TryToOpen
    
++ {not LookForSomething} [Look for something to cut the "metal fangs"]
    ->LookForSomething
...
+++[Stop It]
    #speaker:
    It's clear that not only the glass wasn’t able to cut metal as it would kill the bird if he continued.
...
Roger, roger!
#speaker:
They both rush towards the bushes and start to collect small branches.
A couple of minutes later, they return with half a dozen dry branches.
#speaker:Tails #portrait:tails_sad #layout:left:tails_sad
Hold still, friend.
#speaker:
...
With "two-legged giants" all around.
#speaker:Tails #portrait:tails_scared
